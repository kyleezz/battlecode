package diagonallyfixcheckerboardnuke;
...
public class NormalDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (rc.getRoundNum() > 201 && nearestEnemy != null && (duckState == microState.ATTACKING || duckState == microState.DEFENDING || duckState == microState.RETREATING)) {

            if (rc.getLocation().distanceSquaredTo(nearestFriendlyBase) < 25 && enemies.length + 2 > friends.length && minDistToTrap > 1) {
                Direction enemyDir = rc.getLocation().directionTo(nearestEnemy.getLocation());
                Direction[] trapDirs = {
                        enemyDir,
                        enemyDir.rotateLeft(),
            ...
            } else if (enemies.length > 6  && minDistToTrap > 4 && rc.getCrumbs() > 500) {
                Direction enemyDir = rc.getLocation().directionTo(nearestEnemy.getLocation());
                Direction[] trapDirs = {
                        enemyDir,
                        enemyDir.rotateLeft(),
            ...
            } else if (rc.getCrumbs() > 2000  && minDistToTrap > 4) {
                Direction enemyDir = rc.getLocation().directionTo(nearestEnemy.getLocation());
                Direction[] trapDirs = {
                        enemyDir,
                        enemyDir.rotateLeft(),
        ...
        if (rc.getRoundNum() == 201 && enemies.length > 6) {
            if (rc.canMove(rc.getLocation().directionTo(nearestFriendlyBase))) {
                rc.move(rc.getLocation().directionTo(nearestFriendlyBase));
            }
            targetEnemy = getAttackableTarget();
            while (targetEnemy != null && rc.canAttack(targetEnemy.getLocation())) {
                indicator += "attacked, ";
        ...
        if (rc.getRoundNum() == 202 && enemies.length > 6) {
            targetEnemy = getAttackableTarget();
            while (targetEnemy != null && rc.canAttack(targetEnemy.getLocation())) {
                indicator += "attacked, ";
                rc.attack(targetEnemy.getLocation());
        ...
        MapInfo[] surrounding = rc.senseNearbyMapInfos(4);
        boolean trap = false;
        for (int i = 0; i < surrounding.length; i++) {
            if (surrounding[i].getTrapType() != TrapType.NONE) trap = true;
        }

    ...
    public void flagStuff() throws GameActionException {
        ...
        for (Direction dir : Constants.directions) {
            ...
            for (MapInfo m : surroundings) {
                if (m.isSpawnZone()) score += 100000;
                else if (!m.isWall()) score += 1000000;
            }
            for (MapLocation friendBase : baseLocs)
            score -= Math.sqrt(newFlagLoc.distanceSquaredTo(symmetry.getSymmetricLoc(friendBase)));
            score -= newFlagLoc.distanceSquaredTo(targetEnemyBase);
            if (score < bestScore) {
                bestScore = score;
                bestDir = dir;
            }

...
public class NormalDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (rc.getRoundNum() <= 200) {
            ...
            movement.moveTo(target, rc.getCrumbs() > 800);
            symmetry.findSymmetry();
            rc.setIndicatorString(indicator);
            return;
        }
        ...
        if (nearestEnemy != null) {
            ...
            if (rc.getCrumbs() > 4000 && rc.getRoundNum() > 300) {
                tryTrap(2, 4);
            } else if (minDistToEnemy > 4 && minDistToEnemy < 16 && rc.getCrumbs() > 800) {
                tryTrap(4, 6);
        ...
        if (duckState == microState.ATTACKING) {
            if (rc.isActionReady()) {
                ...
                if (enemies.length * 2 <= (friends.length + 1) * 3) moveAttack();
                else retreat();
            } else {
    ...
    public void neutralMove() throws GameActionException {
        ...
        if (rc.getLocation().distanceSquaredTo(nearestFriendlyBase) <= 20 && friendlyFlags.length != 0){// && Clock.getBytecodesLeft() > 5000) {
            ...
            for (RobotInfo friend : friends) {
                if (friend.getBuildLevel() >= 4) continue;
        ...
        for (RobotInfo friend: friends) {
            if (friend.hasFlag() || friend.getBuildLevel() >= 4) continue;
    ...
    public RobotInfo getHealableFriend() throws GameActionException {
        ...
        for (RobotInfo friend : friends) {
            ...
            if (friend.hasFlag()) score += 1000000000;
            if (friend.getBuildLevel() >= 4) score += 100000000;
    ...
    public void sense() throws GameActionException {
        ...
        for (RobotInfo friend : friends) {
            ...
            if (friend.getBuildLevel() >= 4) builderPals ++;
        }

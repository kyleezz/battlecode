...
public class NormalDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (duckState == microState.ATTACKING) {
            if (rc.isActionReady()) {
                ...
                if (enemies.length * 2 <= (friends.length + 1) * 3) {
                    if (higherHealth * 3 < friends.length * 2) moveAttack();
                    else retreat();
                }
                else retreat();
            } else {
        ...
        // heal
        //if (rc.getExperience(SkillType.HEAL) < 99 || rc.getLevel(SkillType.ATTACK) >= 4) { //ain't specialising in heal
    ...
    public RobotInfo getHealableFriend() throws GameActionException {
        ...
        for (RobotInfo friend : friends) {
            ...
            //else if (friend.getHealth() + rc.getHealAmount() > 1000) score = 1;
            int score = -friend.getHealth() + Util.getRobotAttackDamage(friend);
            if (friend.hasFlag()) score += 1000000;
    ...
    public void sense() throws GameActionException {
        ...
        for (RobotInfo friend : friends) {
            ...
            if (rc.getHealth() < friend.getHealth()) higherHealth++;
        }

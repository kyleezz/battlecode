...
public class NormalDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (nearestEnemy != null && (duckState == microState.ATTACKING || duckState == microState.DEFENDING || duckState == microState.RETREATING)) {
            ...
            MapInfo[] mapInfos = rc.senseNearbyMapInfos(16);
            ArrayList<MapLocation> validPoints = new ArrayList<>();

            for (int i = 0; i < 9; i++) {
                MapLocation temp = null;
                if (i == 8) temp = rc.getLocation();
                else temp = rc.getLocation().add(Constants.directions[i]);

                boolean invalid = false;
                for (int j = 0; j < mapInfos.length; j++) {
                    if (mapInfos[j].getTrapType() != TrapType.NONE) {
                        if (mapInfos[j].getMapLocation().distanceSquaredTo(temp) < 3) invalid = true;
                    }
                }
                if (!invalid) validPoints.add(temp);
            }
            ...
            if (enemies.length > 6) {
                ...
                for (Direction trapDir : trapDirs) {
                    if (validPoints.contains(rc.getLocation().add(trapDir))) {

...
public class NormalDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (duckState == microState.ATTACKING) {
            if (rc.isActionReady()) {
                ...
                if (enemies.length * 2 <= (friends.length + 1) * 3 || (closestFriendlyFlag != null && rc.getLocation().distanceSquaredTo(closestFriendlyFlag.getLocation()) <= 4)) moveAttack();
                else retreat();
            } else {
    ...
    public void neutralMove() throws GameActionException {
        ...
        if (targetLocation == null || rc.getRoundNum() % 10 != 0) 
        ...
        if (rc.getLocation().distanceSquaredTo(nearestFriendlyBase) < 20 || rc.getRoundNum() - lotsaFriends < 10 || bestie == null || closeFriends > Constants.CLOSE_FRIEND_SIZE) {

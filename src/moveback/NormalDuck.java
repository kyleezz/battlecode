...
public class NormalDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (rc.isSpawned()) {
            ...
            if (rc.getRoundNum() > 201 && rc.getLocation().distanceSquaredTo(nearestFriendlyBase) <= 20) {
                int found = 0;
                for (int i = 0; i < 3; i++) {
        ...
        if (duckState == microState.CHASING) {
            ...
            /*if (nearbyTraps >= 2 && rc.getLocation().distanceSquaredTo(nearestFriendlyBase) > 64 && (targetEnemyFlag == null || rc.getLocation().distanceSquaredTo(targetEnemyFlag) > 25)) {
                moveAway();
                //kite();
            }*/
            int tmp = senseBombs();
            if (tmp < lastSenseBombs) bombRound = rc.getRoundNum();
            if (tmp >= 3 && rc.getRoundNum() <= 205/*bombRound + 10 <= rc.getRoundNum()*/) {
                moveAway();
            }
            if (rc.isActionReady()) {
    ...
    public void moveAway() throws GameActionException {
        Direction optDir = nearestEnemy.getLocation().directionTo(rc.getLocation());
        Direction[] optDirs = {
            optDir,
            optDir.rotateLeft(),
            optDir.rotateRight(),
    ...
    int senseBombs() throws GameActionException {
        MapInfo[] surroundings = rc.senseNearbyMapInfos();
        int count = 0;
        for (MapInfo info : surroundings) {
            if (info.getTrapType() != TrapType.NONE) count++;
        }
        return count;
    }

...
public class NormalDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (rc.getRoundNum() < Constants.FIND_CRUMBS) {
            ...
            for (FlagInfo ourFlag : friendlyFlags) {
                if (ourFlag.getLocation().distanceSquaredTo(rc.getLocation()) == 0 && rc.canPickupFlag(ourFlag.getLocation())) rc.pickupFlag(ourFlag.getLocation());
            }
        ...
        if (rc.getRoundNum() <= 200) {
            ...
            movement.moveTo(target, rc.getRoundNum()%2 == 0);
            symmetry.findSymmetry();
            rc.setIndicatorString(indicator);
            return;
        }

        if (!rc.hasFlag() && targetEnemyFlag != null && rc.canPickupFlag(targetEnemyFlag)) {
            rc.pickupFlag(targetEnemyFlag);
            movement.resetRetreat();
        } else if (!rc.hasFlag() && enemyFlags.length != 0 && targetEnemyFlag != null && targetEnemyFlag.distanceSquaredTo(rc.getLocation()) < 9 && rc.isActionReady()) {
        ...
        if (nearestEnemy != null) {
            ...
            if (rc.getCrumbs() > 4000 && rc.getRoundNum() > 300) {
                tryTrap(2, 4);
            } else if (minDistToEnemy > 4 && minDistToEnemy < 16 && rc.getCrumbs() > 500) {
                tryTrap(4, 6);
            } else if (rc.senseNearbyFlags(-1, rc.getTeam()).length > 0) {
                tryTrap(2, 3);
            }

            // if ((rc.getCrumbs() > 4000 && rc.getRoundNum() > 300) || closestFriendlyFlag != null) {
            //     tryTrap(2, 4);
            // } else if (rc.getCrumbs() > 1000 && rc.getRoundNum() < 210) {
            //     tryTrap(8, 2);
            // } 
        }
        ...
        if (duckState == microState.ATTACKING) {
            if (rc.isActionReady()) {
                ...
                if (enemies.length * 2 <= (friends.length + 1) * 3) {
                    if (rc.getHealth() > 300) moveAttack();
                    else kite();
                }
                else retreat();
            } else {
    ...
    public void neutralMove() throws GameActionException {
        ...
        if (friendWithFlag != null) {
            ...
            if (rc.getLocation().distanceSquaredTo(friendWithFlag.getLocation()) > Constants.ESCORT_DISTANCE) {
                if (closer < Constants.ESCORT_SIZE) {
                    movement.moveTo(friendWithFlag.getLocation(), false);
        ...
        int sentinelDist = rc.getLocation().distanceSquaredTo(nearestFriendlyBase) * 100000 + rc.getID();
        if (rc.getLocation().distanceSquaredTo(nearestFriendlyBase) <= 20 && friendlyFlags.length != 0){// && Clock.getBytecodesLeft() > 5000) {
            ...
            for (RobotInfo friend : friends) {
                int dist = friend.getLocation().distanceSquaredTo(nearestFriendlyBase) * 100000 + friend.getID();
            ...
            if (sentinel == null) {
                ...
                if (rc.getLocation().distanceSquaredTo(closestFriendlyFlag.getLocation()) != 0 && rc.getLocation().distanceSquaredTo(nearestFriendlyBase) != 0) movement.moveTo(nearestFriendlyBase, true);
        ...
        if (rc.getRoundNum() - lotsaFriends < 10 || bestie == null || closeFriends > Constants.CLOSE_FRIEND_SIZE) {
            ...
            movement.moveTo(bestie.getLocation(), false);
    ...
    public RobotInfo getHealableFriend() throws GameActionException {
        ...
        for (RobotInfo friend : friends) {
            ...
            if (friend.hasFlag()) score += 10000000;
            if (friend.getBuildLevel() >= 3) score += friend.getBuildLevel() * 10000;

...
public class BuilderDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (rc.getExperience(SkillType.BUILD) < 30 && rc.getCrumbs() > 500) {
            movement.moveRandom();
            if (rc.senseNearbyFlags(4, rc.getTeam()).length == 0) {
                for (int i = 0; i < 8; i++) {
                    MapLocation temp = rc.getLocation().add(Constants.directions[i]);
                    if (((temp.x % 2) ^ (temp.y % 2)) == 1) {
                        if (rc.canDig(temp)) rc.dig(temp);
                    }
                }
            }
        }
        ...
        if (nearestEnemy != null) {
            ...
            if (rc.getCrumbs() > 2000 && rc.getRoundNum() > 300) {
                tryTrap(2, 4);
            } else if (minDistToEnemy > 4 && minDistToEnemy < 16 && rc.getCrumbs() > 250) {
                tryTrap(4, 6);

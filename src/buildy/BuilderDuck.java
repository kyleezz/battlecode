...
public class BuilderDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (nearestEnemy != null) {
            if (rc.getCrumbs() > 1000 && rc.getRoundNum() < 210) {
                tryTrap(2, 4);
            }
            else if (rc.getCrumbs() > 2000 && rc.getRoundNum() > 300) {
                tryTrap(2, 4);
                tryTrap(2, 4);
        ...
        if (duckState == microState.ATTACKING) {
            if (rc.isActionReady()) {
                if (rc.getHealth() < Constants.IDEAL_HEALTH) retreat();
                else if (minDistToEnemy > 10) moveAttack();
                else kite();
            }

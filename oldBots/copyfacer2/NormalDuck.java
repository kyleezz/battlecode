...
public class NormalDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (duckState == microState.ATTACKING) {
            if (rc.isActionReady()) {
                ...
                if (enemies.length * 2 > (friends.length + 1) * 3) kite(true);
                if (rc.getHealth() <= 150) kite(true);
                else moveAttack();
            } else {
                // already attacked, kite away from enemy
                kite(rc.getActionCooldownTurns() >= 20);
            }
    ...
    public void kite(boolean yeet) throws GameActionException {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (yeet) score = attack + -closest * 10000;
            if (score < bestScore) {
                bestScore = score;
                bestDir = Constants.directions[i];
            } else if (score == bestScore) {

...
public abstract class Duck {
    ...
    public MapLocation findNearestCrumb(int radius) throws GameActionException {
        ...
        for (int i = 0; i < crumbs.length; i++) {
            if (rc.senseMapInfo(crumbs[i]).isDam()) continue;
    ...
    void tryWriteEnemyFlag(FlagInfo flag) throws GameActionException {
        for (int flagIndex : Constants.FOUNDENEMYFLAGS) {
            if (flag.getID() == rc.readSharedArray(flagIndex)) return;
        }

        for (int i = 0; i < 3; i++) {
            if (rc.readSharedArray(Constants.ENEMYFLAGINDEX[i]) != 0) continue;
            rc.writeSharedArray(Constants.ENEMYFLAGINDEX[i], Util.serializeLoc(flag.getLocation()));
            rc.writeSharedArray(Constants.FOUNDENEMYFLAGS[i], flag.getID());
        }
    }

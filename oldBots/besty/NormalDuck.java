...
public class NormalDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (rc.hasFlag()) {
            for (int i : Constants.FLAGHELPLOC) {
                MapLocation tmp = Util.deserializeTimedLoc(rc.readSharedArray(i));
                if (tmp == null || rc.getLocation().distanceSquaredTo(tmp) <= 2) {
                    rc.writeSharedArray(i, Util.serializeTimedLoc(rc.getLocation()));
                    break;
                }
            }

            target = nearestSpawn;
            movement.moveTo(target, false);
        }
        ...
        if(!micro.play()) {
            ...
            action.attack();
            action.heal();
            if (rc.isActionReady() && rc.getHealth() <= 900) {
    ...
    public void bestieMove() throws GameActionException {
        for (int i : Constants.FLAGHELPLOC) {
            MapLocation tmp = Util.deserializeTimedLoc(rc.readSharedArray(i));
            if (tmp != null && rc.getLocation().distanceSquaredTo(tmp) <= 45 && rc.getLocation().distanceSquaredTo(tmp) > 2) {
                rc.setIndicatorLine(rc.getLocation(), tmp, 255, 0, 0);
                movement.moveTo(tmp, true);
                return;
            }
        }

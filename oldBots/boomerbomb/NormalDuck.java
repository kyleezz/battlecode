...
public class NormalDuck extends Duck {
    ...
    static MapLocation toHelp = null;
    ...
    public void run() throws GameActionException {
        ...
        if (nearestEnemy != null) {
            ...
            ArrayList<MapLocation> validLocs = new ArrayList<>();
            MapInfo mapInfo[] = rc.senseNearbyMapInfos(5);

            for (int i = 0; i < 9; i++) {
                MapLocation loc = rc.getLocation();
                if (i < 8) {
                    loc = loc.add(Constants.directions[i]);
                }

                boolean valid = true;
                for (MapInfo temp : mapInfo) {
                    if (temp.getTrapType() != TrapType.NONE) {
                        if (temp.getMapLocation().distanceSquaredTo(loc) < 2) valid = false;
                    }
                }
                if (valid) validLocs.add(loc);
            }
            ...
            MapLocation trapLoc = new MapLocation(totalx / enemies.length, totaly / enemies.length);

            if (minDistToTrap > 4 && (enemies.length > friends.length || enemies.length > 6) && rc.getCrumbs() > 500) {
                Direction enemyDir = rc.getLocation().directionTo(trapLoc);
                Direction[] trapDirs = {
                        enemyDir,
                        enemyDir.rotateLeft(),
            ...
            } else if (rc.senseNearbyFlags(16, rc.getTeam()).length > 0 && enemies.length > 4 && minDistToTrap > 1) {
                Direction enemyDir = rc.getLocation().directionTo(trapLoc);
                Direction[] trapDirs = {
                        enemyDir,
                        enemyDir.rotateLeft(),
        ...
        toHelp = null;
        for (int i = 0; i < 3; i++) {
            int temp = rc.readSharedArray(Constants.helpOffset + i);
            if (temp != 0) {
                MapLocation loc = Comms.decodeLoc(temp);
                if (toHelp == null) toHelp = loc;
                else {
                    if (rc.getLocation().distanceSquaredTo(loc) < rc.getLocation().distanceSquaredTo(toHelp)) {
                        toHelp = loc;
                    }
                }
            }
...
//        System.out.println(toHelp);
    ...
    public void moveAttack() throws GameActionException { //omega sus
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (closest == 2) closest = 1; //same thing
    ...
    public RobotInfo getAttackableTarget() throws GameActionException {
        ...
        for (RobotInfo enemy : enemies) {
            ...
            if (rc.hasFlag()) score -= 1000; //also sus
    ...
    public void sense() throws GameActionException {
        ...
        for (int i = 0; i < 3; i++) {
            int temp = rc.readSharedArray(Constants.helpOffset + i);
            if (temp != 0 && ((rc.getLocation().distanceSquaredTo(Comms.decodeLoc(temp)) == 1 && enemies.length == 0 && rc.isSpawned() && rc.getRoundNum() - 2 > spawnTurn) || rc.senseNearbyFlags(-1, rc.getTeam()).length == 0)) {
                System.out.println("bye");
                      rc.writeSharedArray(Constants.helpOffset + i, 0);
            }
        }

        if (rc.getRoundNum() > 2) {
            for (int i = 0; i < 3; i++) {
                MapLocation temp = baseLocs[i];
                if (temp == null) break;
                if (rc.canSenseLocation(temp) && rc.senseNearbyFlags(-1, rc.getTeam()).length > 0 && (enemies.length > 4 || enemies.length > friends.length)) {
                    boolean written = false;
                    for (int j = 0; j < 3; j++) {
                        int tempnum = rc.readSharedArray(Constants.helpOffset + j);
                        if (tempnum == 0) continue;
                        if (Comms.decodeLoc(tempnum).equals(temp)) {
                            written = true;
                        }
                    }
                    if (!written) {
                        ...
                        for (int j = 0; j < 3; j++) {
                            if (rc.readSharedArray(Constants.helpOffset + j) == 0) {
                                rc.writeSharedArray(Constants.helpOffset + j, Comms.encode(baseLocs[i], 0));
                                break;
                            }
                        }
                    }

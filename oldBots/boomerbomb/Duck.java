...
public abstract class Duck {
    ...
    public MapLocation findSpawnLocation(boolean goHelp) throws GameActionException {
        ...
        for (int i = 0; i < 27; i++) {
            ...
            if (goHelp && rc.getID() % 3 == 0) {
                for (int j = 0; j < 3; j++) {
//                    if (rc.readSharedArray(Constants.BASELOCINDEX[j]) == 0) continue;
                    if (rc.readSharedArray(Constants.helpOffset + j) == 0) continue;
                    MapLocation temp = Comms.decodeLoc(rc.readSharedArray(Constants.helpOffset + j));
                    if (spawnLoc.distanceSquaredTo(temp) <= 8) {

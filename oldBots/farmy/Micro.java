...
public class Micro {
    ...
    boolean play() throws GameActionException {        
        ...
        if (flagTarget != null && rc.canPickupFlag(flagTarget)) rc.pickupFlag(flagTarget);
        ...
        // get best spot
        MicroInfo bestLocation = microInfos[0];
        for (int i = 1; i < 9; i++) {
        ...
        if (enemies.length >= 5 && (microState == STATE.DEFENSIVE || microState == STATE.EVASIVE)) {
            if (rc.getCrumbs() > Constants.MINCRUMBS && rc.canBuild(TrapType.EXPLOSIVE, rc.getLocation())) {

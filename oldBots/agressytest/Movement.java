...
public class Movement extends Util {
    ...
    Movement(Symmetry symmetry, RobotController rc) throws GameActionException {
...
//        bug.goTo(target);
    ...
    void moveToBroadcast() throws GameActionException {
        /*MapLocation tmp = knownFlag();
        if (tmp != null && rc.getLocation().distanceSquaredTo(tmp) <= 900) {
            rc.setIndicatorLine(rc.getLocation(), tmp, 255, 255, 255);
            boolean flagTaken = false;
            for (int i = Constants.FLAGSTART; i < Constants.FLAGSEND; i++) {
                if (Util.deserializeLoc(rc.readSharedArray(i)) == tmp) flagTaken = true;
            }
            if (!flagTaken) {
                moveTo(tmp, true);
                return;
        ...
        for (MapLocation flagLoc : flagLocs) {
            if (rc.getLocation().distanceSquaredTo(flagLoc) < mini) {
                ...
                for (MapLocation broadcastLoc : broadcastLocs) {
                    ...
                    if (broadcastLoc.distanceSquaredTo(flagLoc) <= 10) inRange = true;
                }
                if (!inRange) continue;

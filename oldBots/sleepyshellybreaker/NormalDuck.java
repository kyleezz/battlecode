...
public class NormalDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        if (enemies.length == 0) {
            ...
            if (friends.length < Optimise.MIN_TEAM_SIZE || teamHealth / (friends.length+1) < Optimise.LOW_HEALTH_RETREAT) {
                duckState = microState.RETREATING;
            }
        } else {
            if (friends.length < Optimise.MIN_TEAM_SIZE || (enemyTeamDamage != 0 && enemyTeamHealth/teamDamage > teamHealth/enemyTeamDamage+Optimise.ENEMY_TURN_DIFF)) {
        ...
        if (duckState == microState.CHASING) {
            ...
            if (minDistToEnemy > 2 && enemyFlags.length != 0) autofill();
            if (rc.isActionReady()) {
    ...
    public void neutralMove() throws GameActionException {
        ...
        if (friendWithFlag != null) {
            ...
            if (closer < Optimise.ESCORT_SIZE) {
        ...
        for (RobotInfo friend: friends) {
            if (friend.hasFlag()) continue;
            int score = friend.getLocation().distanceSquaredTo(targetLocation) * 1000 - friend.getHealth();
    ...
    public void moveAttack() throws GameActionException {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            int score = attack - Math.abs(4 - closest) * 100000;
            if (score < bestScore) {
                bestScore = score;
    ...
    public void kite() throws GameActionException {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            int score = attack + Math.abs(Optimise.KITE_DISTANCE-closest) * 10000;
            if (score < bestScore) {
                bestScore = score;
                bestDir = Constants.directions[i];
            }
    ...
    public void retreat() throws GameActionException {
        if (enemies.length != 0 && minDistToEnemy < Optimise.RETREAT_ENEMY_RADIUS && !rc.hasFlag()) {
    ...
    public void autofill() throws GameActionException {
    ...
    public RobotInfo getHealableFriend() throws GameActionException {
        ...
        for (RobotInfo friend : friends) {
            ...
            int score = Math.abs(Optimise.IDEAL_HEALTH - friend.getHealth());

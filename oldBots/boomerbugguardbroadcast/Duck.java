...
public abstract class Duck {
    ...
    public void recordFlagPickup(FlagInfo flag) throws GameActionException {
        for (int i = 0; i < 3; i++) {
            if (rc.readSharedArray(Constants.ENEMYFLAGINDEX[i]) == 0 || rc.readSharedArray(Constants.ENEMYFLAGINDEX[i]) == flag.getID()) {
                rc.writeSharedArray(Constants.ENEMYFLAGINDEX[i], flag.getID());
                rc.writeSharedArray(Constants.ENEMYFLAGLOC[i], Comms.encode(flag.getLocation(), rc.getRoundNum()%10));
                return;
            }
        }
    }
    ...
    public MapLocation findNearestEnemyFlag() throws GameActionException {
        ...
        for (int i = 0; i < broadcasts.length; i++) {
            ...
            for (int j = 0; j < 3; j++) {
                if (baseLocs[j] == null ||
                baseLocs[j].equals(Comms.decodeLoc(rc.readSharedArray(Constants.ENEMYFLAGLOC[0]))) ||
                baseLocs[j].equals(Comms.decodeLoc(rc.readSharedArray(Constants.ENEMYFLAGLOC[1]))) ||
                baseLocs[j].equals(Comms.decodeLoc(rc.readSharedArray(Constants.ENEMYFLAGLOC[2])))) continue;

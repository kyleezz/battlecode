package boomerbugguardbroadcast;
...
public class NormalDuck extends Duck {
    ...
    static FlagInfo nearestEnemyFlag;
    ...
    FlagInfo pickedUpFlag = null;
    int gotFlagRound = -1;

    MapLocation gotFlagFrom = null;
    ...
    public void run() throws GameActionException {
        ...
        if (rc.isSpawned()) {
            ...
            if (rc.hasFlag()) {
                ...
                gotFlagRound = rc.getRoundNum();
                if (rc.getLocation().distanceSquaredTo(nearestFriendlyBase) < 4) {
                    pickedUpFlag = null;
                    gotFlagFrom = null;
                }
            }
        ...
        if (!rc.isSpawned()) {
            ...
            if (pickedUpFlag != null && rc.getRoundNum() - gotFlagRound > 4) {
                // had flag and died lol
                for (int i = 0; i < 3; i++) {
                    if (rc.readSharedArray(Constants.ENEMYFLAGINDEX[i]) == pickedUpFlag.getID()) {
                        if (Comms.decodeLoc(rc.readSharedArray(Constants.ENEMYFLAGLOC[i])).equals(pickedUpFlag.getLocation())) {
                            rc.writeSharedArray(Constants.ENEMYFLAGINDEX[i], 0);
                            rc.writeSharedArray(Constants.ENEMYFLAGLOC[i], 0);
                        }
                    }
                }
                pickedUpFlag = null;
        ...
        if (rc.getRoundNum() < Constants.FIND_CRUMBS) {
            if (friendlyFlags.length != 0 && !rc.isLocationOccupied(friendlyFlags[0].getLocation())) {
                movement.moveTo(friendlyFlags[0].getLocation(), true);
                return;
            }
        ...
        if (targetEnemyFlag != null && rc.canPickupFlag(targetEnemyFlag)) {
            ...
            recordFlagPickup(nearestEnemyFlag);
            pickedUpFlag = nearestEnemyFlag;
    ...
    public boolean onBase() throws GameActionException {
        if (rc.getRoundNum() == 1) return false;
        MapLocation loc = rc.getLocation();
        for (MapLocation baseCenter : baseLocs) {
            if (loc.equals(baseCenter)) return true;
        }
        return false;
    }
    ...
    public void sense() throws GameActionException {
        ...
        int closestFlagDist = 10000000;
        nearestEnemyFlag = null;
        for (FlagInfo flag : enemyFlags) {
            int flagDist = rc.getLocation().distanceSquaredTo(flag.getLocation());
            if (flagDist < closestFlagDist) {
                closestFlagDist = flagDist;
                nearestEnemyFlag = flag;
            }
        }

...
public class Movement {
    ...
    void moveTo(MapLocation target) throws GameActionException {
        ...
        for (int i = 0; i < 16; i++) {
            if (rc.canMove(dir)) {
                rc.move(dir);
                return;
            } else if (rc.hasFlag() && rc.canFill(rc.getLocation().add(dir))) {
    ...
    void moveToBroadcast() throws GameActionException {
        ...
        //     Util.getSymmetricLoc(Util.deserializeLoc(rc.readSharedArray(0))),
        //     Util.getSymmetricLoc(Util.deserializeLoc(rc.readSharedArray(1))),
        //     Util.getSymmetricLoc(Util.deserializeLoc(rc.readSharedArray(2))),
        // };
        ...
        for (MapLocation flagLoc : flagLocs) {
            if (rc.getLocation().distanceSquaredTo(flagLoc) < mini) {
                ...
                for (int i = 4; i < 7; i++) {
                    if (Util.deserializeLoc(rc.readSharedArray(i)) == flagLoc) flagTaken = true;
                }
                if (flagTaken) continue;

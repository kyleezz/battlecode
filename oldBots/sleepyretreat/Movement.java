...
//        bug.goTo(target);
    ...
    void fastRetreat(MapLocation nearestSpawn) throws GameActionException {
        ...
        if ((!rc.hasFlag() || rc.getLocation().distanceSquaredTo(nearestSpawn) > 20) && bestDir != null) {
        ...
        for (Direction dir : dirs) {
            if (!rc.canMove(dir) || globalbfs.getVal(rc.getLocation().add(dir)) > retreatBest) continue;
            int danger = 0;
            for (RobotInfo enemy : enemies) {
                if (rc.getLocation().add(dir).distanceSquaredTo(enemy.getLocation()) < 16) danger ++;
            }
            if (danger < leastEnemies) {
                leastEnemies = danger;
                bestDir = dir;
            } else if (danger == leastEnemies && (dir.dx + dir.dy) < bestDir.dx + bestDir.dy) {
                bestDir = dir;
            }

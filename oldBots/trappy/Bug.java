...
public class Bug {
    ...
    private static boolean move(Direction dir) throws GameActionException {
        ...
        if (Globals.shouldDig && rc.senseMapInfo(rc.getLocation().add(dir)).isWater() && rc.canFill(rc.getLocation().add(dir))) {
    ...
    private static boolean canMove(Direction dir) throws GameActionException {
        ...
        if (rc.getCrumbs() >= 10 && rc.getActionCooldownTurns() < 10) {
            return rc.canMove(dir) || rc.senseMapInfo(rc.getLocation().add(dir)).isWater();

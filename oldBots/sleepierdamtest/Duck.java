...
public abstract class Duck {
    ...
    public MapLocation findSpawnLocation(boolean goHelp) throws GameActionException {
        ...
        for (int i = 0; i < 27; i++) {
            ...
            if (rc.getRoundNum() > 200) {
                boolean skip = false;
                for (int j = 0; j < 3; j++) {
                    if (spawnLoc.distanceSquaredTo(baseLocs[j]) <= 2) {
                        if (rc.readSharedArray(Constants.flagGoneOffset + j) == 1) skip = true;
                    }
                }
                if (skip) continue;
            }
    ...
    public MapLocation findNearestFriendlyBase() throws GameActionException {
        ...
        int closest = 1000000;
        for (int i = 0; i < 3; i++) {
            if (rc.readSharedArray(i + Constants.flagGoneOffset) == 1) continue;

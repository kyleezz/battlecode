...
public class NormalDuck extends Duck {
    ...
    public void run() throws GameActionException {
        ...
        for (int i = 0; i < 3; i++) {
            if (rc.getLocation().distanceSquaredTo(baseLocs[i]) < 20) {
                boolean safe = flags.length > 0;
                if (rc.canWriteSharedArray(0, 0)) {
                    if (safe) rc.writeSharedArray(Constants.flagGoneOffset + i, 0);
                    else  rc.writeSharedArray(Constants.flagGoneOffset + i, 1);
                }
            }
        }
        ...
        if (nearestEnemy != null && (duckState == microState.ATTACKING || duckState == microState.DEFENDING || duckState == microState.RETREATING)) {
            if (rc.getLocation().distanceSquaredTo(nearestFriendlyBase) <= 16 && (enemies.length + 3 > friends.length || enemies.length > 6) && minDistToTrap > 1) {
            ...
            if (enemies.length > 8 && minDistToTrap > 6 && rc.getCrumbs() > 1500) {
                Direction enemyDir = rc.getLocation().directionTo(nearestEnemy.getLocation());
                Direction[] trapDirs = {
                        enemyDir,
                        enemyDir.rotateLeft(),
            ...
            } else if (rc.getCrumbs() > 2000  && minDistToTrap > 6) {
                Direction enemyDir = rc.getLocation().directionTo(nearestEnemy.getLocation());
                Direction[] trapDirs = {
                        enemyDir,
                        enemyDir.rotateLeft(),
    ...
    public void neutralMove() throws GameActionException {
        ...
        if (goToHelp != null && (bestVal > 0.1 && rc.getLocation().distanceSquaredTo(goToHelp) < 400)) {

...
// build - build traps or fill water
        if (nearestEnemy != null) {
            ...
            MapLocation target = new MapLocation(sumx / enemies.length, sumy / enemies.length);

            MapInfo[] traps = rc.senseNearbyMapInfos(-1);

            int closestDist = 1000000;

            for (MapInfo temp : traps) {
                if (temp.getTrapType() != TrapType.NONE) {
                    closestDist = Math.min(closestDist, temp.getMapLocation().distanceSquaredTo(target));
                }
            }

            if  ((enemies.length > friends.length || enemies.length > 6) && rc.getCrumbs() > 500) {
                ...
                for (Direction trapDir : trapDirs) {
                    if (minDistToTrap > 4 || rc.getLocation().add(trapDir).distanceSquaredTo(target) < closestDist) {
    ...
    public RobotInfo getHealableFriend() throws GameActionException {
        ...
        for (RobotInfo friend : friends) {
            ...
            if (friend.getHealth() + rc.getHealAmount() > 1000) score = 1; //sus
    ...
    public RobotInfo getAttackableTarget() throws GameActionException {
        ...
        for (RobotInfo enemy : enemies) {
            ...
            if (enemy.getHealth() <= totalCanAttack / 2) return enemy; //sus
